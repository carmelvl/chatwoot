# Kita: thread metadata from kita-grip-sync (AI title, Grip ticket link, display id, priority, status,
# owner) keyed by the thread's root desk message.
class Api::V1::Kita::ThreadsController < ApplicationController
  include ::Kita::BridgeRequest

  THREAD_FIELDS = %i[title ticket_id ticket_url ticket_display_id ticket_priority ticket_status ticket_owner].freeze
