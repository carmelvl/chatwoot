# Kita: thread metadata from kita-grip-sync (AI title, Grip ticket link) keyed by the thread's root desk message.
class Api::V1::Kita::ThreadsController < ApplicationController
  ...
  def create
    ...
    thread.update!(account: account, conversation: conversation, **params.permit(:title, :ticket_id, :ticket_url).to_h.symbolize_keys)
