...
const numbers: WhatsAppNumber[] = [
  // legacy per-number inbox fields are still accepted (and ignored: everything goes to the Customers inbox)
  { phoneNumberId: '111111111111111', inboxIdentifier: 'IN_WA_CARMEL', webhookSecret: 'cw-wa-carmel', ownerName: 'Carmel Limcaoco' },
  { phoneNumberId: '222222222222222', ownerName: 'Rhea Malhotra', agentAccessToken: 'RHEA_TOKEN' },
];
...
test('customer message -> incoming (Customers inbox); contact whatsapp:+E164; channel_key for Grip', () => {
  const { items, skipped } = parseWhatsAppWebhook(fixture('wa_messages_text.json'), numbers);
  assert.deepEqual(skipped, []);
  assert.equal(items.length, 1);
  const it = items[0];
  assert.equal(it.kind, 'customer');
  assert.equal('inboxIdentifier' in it.message, false);
...
test('phone-app echo -> business echo for the customer in `to`; multiple numbers, one inbox; revoke skipped', () => {
  ...
  assert.equal(b.items[0].number.ownerName, 'Rhea Malhotra');
...
function world() {
  ...
  const appFetch = (async (u: any, init: any) => {
    if (String(u).endsWith('/custom_attributes')) return Response.json({});
  ...
  const bridge = new Bridge({ store, chatwoot: cw.client, inbox: 'IN_CUSTOMERS', senders: {}, publicUrl: PUBLIC_URL, app, desk, fetchImpl });
...
test('e2e: customer message then phone-app echo land in the same conversation (incoming, then outgoing as the owner)', async () => {
  ...
  await settle();
  // unlinked number: the conversation's contact is the channel; Maria is the message's sender
  assert.equal(w.cw.calls.find((c) => c.path.endsWith('/contacts'))!.body.identifier, 'whatsapp-channel:whatsapp:+639998887777');
  const contact = w.cw.calls.find((c) => c.path === '/public/api/v1/inboxes/IN_CUSTOMERS/contacts' && c.body.identifier === 'whatsapp:+639998887777')!;
...
test('e2e: echo to a new customer creates the conversation, authored by the number\'s owner; media attached', async () => {
  ...
  await settle();
  assert.ok(w.cw.calls.some((c) => c.path === '/public/api/v1/inboxes/IN_CUSTOMERS/contacts' && c.body.identifier === 'whatsapp-channel:whatsapp:+639991112222'));
...
test('mirror mode: an agent typing in the desk sends nothing and gets a private note (once); our own echoes never loop', async () => {
  ...
  const sign = (b: string) => {
    ...
    return { 'x-chatwoot-timestamp': ts, 'x-chatwoot-signature': `sha256=${hmacHex('cw-customers', `${ts}.${b}`)}` };
  };
  const hit = (b: string) => fetch(`${base}/chatwoot/customers`, { method: 'POST', body: b, headers: { 'content-type': 'application/json', ...sign(b) } }).then((r) => r.json());
  ...
  const old = await fetch(`${base}/chatwoot/whatsapp/111111111111111`, { method: 'POST', body, headers: { 'content-type': 'application/json', ...sign(body) } });
...
test('phone-app echo with the owner\'s agentEmail is authored by that desk agent (no prefix, no tokens in the bridge)', async () => {
  ...
  const cw = fakeChatwoot();
  const store = new Store(':memory:');
  const bridge = new Bridge({ store, chatwoot: cw.client, inbox: 'IN_CUSTOMERS', senders: {}, publicUrl: PUBLIC_URL, desk, fetchImpl: cw.fetchImpl });
  ...
  assert.equal(store.isSeen('out:8100'), true); // never sent back out
});
