...
export interface Classification {
  ...
  priority: Priority;
  summary: string;
  /** 3-6 word name of the thread's topic for the desk's thread list (e.g. "Batch 14 scores missing"). Set even when not an issue. */
  thread_title: string;
}
...
export const CLASSIFICATION_SCHEMA = {
  ...
  required: ['is_issue', 'is_new_issue', 'title', 'priority', 'summary', 'thread_title'],
  properties: {
    ...
    thread_title: { type: 'string', description: 'A 3-6 word title naming what this thread is about, e.g. "Batch 14 scores missing". Always set, even when it is not an issue. No trailing punctuation.' },
  },
} as const;
...
The transcript is ONE thread (a top-level message and its replies) from a customer's long-lived conversation; several customer people may speak. Each thread has its own ticket, which always describes the LATEST OPEN issue in the thread.
...
Also give the thread a 3-6 word title naming its topic (e.g. "Batch 14 scores missing"), even when it is not an issue.
Lines marked (thread reply) are replies inside a chat thread; the name before the colon is the person who wrote it.
Write the titles and summary in English for the Kita team, even if the customer writes in another language. Never include phone numbers, emails or account numbers.
...
export function buildUserPrompt(thread: ThreadMessage[], ticket?: Pick<TicketRow, 'title' | 'priority' | 'summary'> & { status?: string }): string {
  ...
  const existing = ticket
    ? `Existing ticket for this thread${ticket.status === 'done' ? ' (resolved)' : ''}:\ntitle: ${ticket.title}\npriority: ${ticket.priority}\nsummary: ${ticket.summary}`
    : 'There is no ticket for this thread yet.';
...
export class ClaudeClassifier {
  ...
  async classify(thread: ThreadMessage[], ticket?: Pick<TicketRow, 'title' | 'priority' | 'summary'> & { status?: string }): Promise<Classification> {
    ...
    if (res.stop_reason === 'refusal') return { is_issue: false, is_new_issue: false, title: '', priority: 'low', summary: '', thread_title: '' };
...
export class OpenAIClassifier {
  ...
  async classify(thread: ThreadMessage[], ticket?: Pick<TicketRow, 'title' | 'priority' | 'summary'> & { status?: string }): Promise<Classification> {
    ...
    if (choice?.message?.refusal) return { is_issue: false, is_new_issue: false, title: '', priority: 'low', summary: '', thread_title: '' };
...
function normalize(o: any): Classification {
  ...
  return { is_issue: o?.is_issue === true, is_new_issue: o?.is_new_issue === true, title: String(o?.title ?? '').slice(0, 200), priority, summary: String(o?.summary ?? ''),
    thread_title: String(o?.thread_title ?? '').replace(/\s+/g, ' ').trim().replace(/[.!?]+$/, '').slice(0, 120) };
}
