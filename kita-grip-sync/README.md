...
* **History imported by kita-bridges** (`content_attributes.kita_backfill`, the whole chat when Kita joins a channel): counted in the snapshot like any message, but never classified per message. Only the most recent thread of the import (the last backfilled message's thread) gets one debounced `classify_backfill:<id>` job, and only if that message is under 7 days old. Old history never creates tickets.
* **Out-of-scope accounts: only with `SCOPE_FILTER=on`.** By default (`SCOPE_FILTER=off`) Grip's `in_scope` changes nothing: every conversation stays open, visible under its account (or Unlinked), is assigned to its DRI and classified like any other. What follows applies with `SCOPE_FILTER=on`. Grip's `POST /support/conversations` answers `in_scope: boolean`, flat or inside Grip's `{ success, data }` envelope (false when the linked account isn't on Grip's Customers page: paused, closed, or no pilot). kita-bridges already drops messages from channels Grip lists as out of scope; this catches what slips through, e.g. a brand-new channel that auto-linked to a paused account.
...
* **Owner in the desk.** Grip's `POST /support/conversations` also answers `dri_email`, `dri_name`, `sales_owner_email` and `account_name` (flat or in `{ success, data }`). After each upsert that carries them, an `owner:<id>` job (`src/owner.ts`):
  ...
  * a DRI who isn't a Chatwoot agent still gets the attributes, and `dri_not_agent` is logged. With `SCOPE_FILTER=on`, out-of-scope conversations (`in_scope: false`) get the attributes but are never assigned; by default they are assigned like any other.
